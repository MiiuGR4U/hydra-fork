...
export class DownloadManager {
  ...
  private static async getBuffdriveDownloadOptions(
    download: Download,
    resumingFilename?: string
  ) {
    const downloadUrl = await BuffdriveApi.getDownloadUrl(download.uri);
    const filename = this.resolveFilename(
      resumingFilename,
      download.uri,
      downloadUrl
  ...
  private static async getGofilecdnDownloadOptions(
    download: Download,
    resumingFilename?: string
  ) {
